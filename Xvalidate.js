(function ($){
  var xOffset = 15; // x 偏移像素
		...
		checkRepeat:function(value){
			...
			var toval=jQuery('input[name="'+to+'"]').eq(0).val();
			 if((toval== "undefined" || toval== "")&&(value!="undefined" && value!="")){
				 ...
				 validator.showTipMsgByAjax(element,errmsg,value==toval);
				 return value==toval;
			...
			var s = $.ajax({
				...
				success: function(data){
					data = data.replace(/(^\s*)|(\s*$)/g, "");
					   ...
					   if(data=='success') {
						    return  validator.showTipMsgByAjax(element,errmsg,true);
					   }
				   }
			 }).responseText;
			 s = s.replace(/(^\s*)|(\s*$)/g, "");
	...
	validator.add=function(name,method){
		validator[name]='this.'+name+'x()';
		validator[name+'x']=method;		
	}

	validator.option=function(options1){
		var idx=options1[0].id;
		if(idx!=undefined&&idx!="undefined"&&idx!=""){
			var ss=eval(options1);
			//alert(ss.length);
			for(var i=0;i<ss.length;i++){
				var id=options1[i].id;
				var optionsx=options1[i].option;
				var validTypex=optionsx.validType;
	...
	validator.showTipMsgByAjax=function (element,msg,isSuccess){
		...
		var mytip= jQuery( '<div class="validator_msg_tip">'+
								   	'<div class="validator_tip_msg_content">'+msg+
									'</div><div class="validator_msg_tip_direction validator_msg_tip_left"></div></div>');
	...
	validator.showTipMsg=function (element,msg,errindex,isSuccess){
		...
		var mytip= jQuery( '<div class="validator_msg_tip"><div ></div>'+
								   	'<div >'+tipmsg+
									'</div><div ></div></div>');
				mytip.css("top", top+"px").css("left", left+"px");
				mytip.insertAfter(element); 
	...
	validator.checkDataType = function(element,datatype){
		...
		// alert(eval(this[datatype]));
			...
			// alert(this[datatype].test(value));
		switch(datatype){
				 case "IdCard" :
			 case "Date" :
			 case "Repeat" :
	...
	validator.check=function(obj){
		var isValid;
		this['valiData'] = $.trim(obj.attr('option'));
		//2013-1-30alert(this['valiData']);
	...
	$.fn.checkForm = function(showType) {
		...
		var elements = form.find(':input[option]');
